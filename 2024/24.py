...
def solve_part2(data):
    ...
    operators_with_or_only = (
            operators_with_or - operations_operators['AND'] - operations_operators['XOR'])
    ...
    for l_operator, operation, r_operator, output in operations:
        match operation:
            case 'XOR':
                is_any_xyz = any(
                    o[0] in {'x', 'y', 'z'} for o in (l_operator, r_operator, output))
                if not is_any_xyz or output in operators_with_or:
                    bad_nodes.add(output)
            case 'AND':
                is_first_bit = len({'x00', 'y00'} & {l_operator, r_operator}) != 0
                if not is_first_bit and output not in operators_with_or_only:
                    bad_nodes.add(output)
            case 'AND' | 'OR':
                is_output_z_but_not_last = (output[0] == 'z' and output != z_list[-1])
