    ...
    for command in code:
        ...
        if map[ny][nx] in {'[', ']'}:
            if dx:
                ...
                left_x, right_x = (nx, nx + 1) if map[ny][nx] == '[' else  (nx - 1, nx)
                stack = [[(ny, left_x, right_x)],]
                layer = 0
                while True:
                    layer += 1
                    ...
                    for _, left_x, right_x in stack[-1]:
                        next_layer_chars |= set(map[next_layer_y][left_x:right_x + 1])
                    if (
                            next_layer_chars == {'.'}
                            or '#' in next_layer_chars
                    ):
                        break
                    for _, left_x, right_x in stack[-1]:
                        match map[next_layer_y][left_x]:
                            case ']':
                                cur_layer.append((next_layer_y, left_x - 1, left_x))
                            case '[':
                                cur_layer.append((next_layer_y, left_x, right_x))
                        if map[next_layer_y][right_x] == '[':
                            cur_layer.append((next_layer_y, right_x, right_x + 1))
                ...
                if next_layer_chars == {'.'}:
                    while stack:
                        ...
                        for box in layer:
                            ...
                            map[box_y + dy][left_x], map[box_y + dy][right_x] = '[', ']'
