...
def solve_part1(data):
    ...
    while heap:
        ...
        for new_direction, (dy, dx) in enumerate(MOVES):
            ny, nx = y + dy, x + dx
            if (maze[ny][nx] == '.' and (ny, nx) not in seen) or (ny, nx) == end:
                rotate = abs(new_direction - direction)
                add_score = 1 + 1000 * (rotate if not rotate % 2 else 1)
                heapq.heappush(heap, (score + add_score, ny, nx, new_direction))
                seen.add((ny, nx))
...
def solve_part2(data):
    ...
    while heap:
        ...
        for new_direction, (dy, dx) in enumerate(MOVES):
            ...
            if maze[ny][nx] == '.':
                rotate = abs(new_direction - direction)
                new_score = score + 1 + 1000 * (rotate if not rotate % 2 else 1)
                if (ny, nx) not in seen or seen[(ny, nx)] >= new_score - 1001:
                    new_path = path + [(ny, nx)]
                    heapq.heappush(heap, (new_score, ny, nx, new_direction, new_path))
