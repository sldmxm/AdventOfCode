...
CODE_KEYS = {
    char: (i // 3, i % 3)
    for i, char in enumerate('789456123 0A')
}
DIRECTION_KEYS = {
    char: (i // 3, i % 3)
    for i, char in enumerate(' ^A<v>')
}
...
class Keypad:
    ...
    def go_to_key(self, key: str) -> str:
        ...
        directions = set()
        if ny > self.y: directions.add('v')
        elif ny < self.y: directions.add('^')
        if nx > self.x: directions.add('>')
        elif nx < self.x: directions.add('<')
        path = []
...
def solve_fast(data, robots_count):
    def get_code_counter(keypad, pre_code, count):
        ...
        for char in pre_code:
            ...
            is_empty_cell_on_path = (
                    y == empty_y and nx == empty_x
                    or x == empty_x and ny == empty_y
            )
    ...
    for code in data:
        ...
        for _ in range(robots_count + 1):
            ...
            for (y, x, is_empty_cell_on_path), count in code_layer_counter.items():
                # moves = ''
                # moves += (-x * '<') if x < 0 else ''
                ...
                #     moves = moves[::-1]
                # moves += 'A'
                # так проще, конечно, - отрицательные значения просто не добавятся в строку
                moves = (
                        ('<' * -x + 'v' * y + '^' * -y + '>' * x)[
                        :: -1 if is_empty_cell_on_path else 1] + 'A'
                )
                step_code_counter = get_code_counter(DIRECTION_KEYS, moves, count)
                for key, count in step_code_counter.items():
                    next_code_layer_counter[key] = next_code_layer_counter.get(key, 0) + count
