...
def get_sides_by_corners(shape):
    ...
    for y, x in shape:
        corners_to_check = {
            (dy, dx) for dy, dx in CORNERS if (y + dy, x + dx) not in shape
        }
        corners = []
        for corner_dy, corner_dx in corners_to_check:
            if (
                (corner_dy + y, x) not in shape
                and (y, corner_dx + x) not in shape
                or (corner_dy + y, x) in shape
                and (y, corner_dx + x) in shape
            ):
        ...
        for x in range(first_col, last_col + 1):
            if (y, x) in shape:
                for trigger, not_in_shape, in_shape in PATTERNS:
                    if (y + trigger[0], x + trigger[1]) not in shape and (
